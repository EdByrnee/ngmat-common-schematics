export function insertEndOfVars(insersion:string, original:string): string{

    // Find the position of the end of the constructor
    let start_of_cons_num = original.search("constructor");

    return original = original.substring(0,start_of_cons_num) + insersion + "\n\n" +  original.substring(start_of_cons_num)
