function insertEndOfConstructor(insersion:string, original:string): string{

    // Find the position of the end of the constructor
    let start_of_cons_num = original.search("constructor");
    let start_of_cons_str = original.substr(start_of_cons_num);
    let end_of_cons_num = start_of_cons_str.search("{") - 3 + start_of_cons_num;

    return original = original.substring(0,end_of_cons_num) + ", " + insersion + original.substring(end_of_cons_num)

}



// let insersion  = 'public: AppService'
...
// let new_ts = insertEndOfConstructor(insersion,original_ts)
// console.log(new_ts);